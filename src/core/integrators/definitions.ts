...
bool tracePrimary(vec3 ro, vec3 rd, out float hitT) {
  ...
  for (int i = 0; i < MAX_TRACE_STEPS; i++) {
    ...
    float d = DE(p) * uIntegrator_fudgeFactor;
    if (d < eps) {
      hitT = t;
      return true;
    }
...
const deSurfaceOptionTemplate: IntegratorOptionDefinition[] = [
...
function buildDeSurfaceOptions(defaultOverrides: Record<string, number>): IntegratorOptionDefinition[] {
  return deSurfaceOptionTemplate.map((option) => ({
    ...option,
...
float rand(inout vec2 state) {
  state = fract(state * vec2(127.1, 311.7) + vec2(269.5, 183.3));
  float n = dot(state, vec2(12.9898, 78.233));
...
vec3 sampleCosineHemisphere(vec3 n, inout vec2 rng) {
  float r1 = rand(rng);
  float r2 = rand(rng);
  float phi = 2.0 * PI * r1;
  float r = sqrt(r2);
...
vec3 sampleGGXHalfVector(vec3 n, float roughness, inout vec2 rng) {
...
float specularLobeProbability(vec3 f0, float roughness) {
  ...
  return clamp(base + glossBoost, 0.05, 0.95);
}
...
void evaluateBSDF(
  ...
  float dielectricF0 = mix(0.02, 0.08, clamp(reflectivity, 0.0, 1.0));
...
vec3 sampleSunDirection(inout vec2 rng, out float pdf) {
...
vec3 renderColor(vec3 ro, vec3 rd) {
  vec2 rng = gl_FragCoord.xy + vec2(float(uSubframe) * 0.6180339 + 1.0, float(uSubframe) * 1.4142135 + 2.0);
  ...
  for (int bounce = 0; bounce < MAX_BOUNCES; bounce++) {
    ...
    vec3 base = clamp(baseColor(hitPos, n), vec3(0.0), vec3(1.0));
    ...
    float dielectricF0 = mix(0.02, 0.08, reflectivity);
...
export const INTEGRATORS: IntegratorDefinition[] = [
  {
    id: "fast-raymarch",
    name: "Fast Raymarch",
    description: "Shared DE surface raytracer tuned for high interactivity.",
    options: buildDeSurfaceOptions({
      detailAOExp: -1.6,
      maxRaySteps: 128,
      maxDistance: 500,
      aoStrength: 0.2,
    ...
    glsl: deSurfaceRaytracerGlsl
  },
  {
    id: "de-raytracer",
    name: "DE Raytracer (Quality)",
    description: "Same DE surface core as Fast Raymarch, with higher quality defaults.",
    options: buildDeSurfaceOptions({
      detailAOExp: -1,
      maxRaySteps: 220,
      maxDistance: 1200,
